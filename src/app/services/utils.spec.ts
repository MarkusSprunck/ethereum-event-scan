...
describe('UtilsService - consolidated', () => {
  ...
  describe('reloadAfterUpdate and fetchABIFromVerifiedContract', () => {
    ...
    it('reloadAfterUpdate schedules a reload (jest timers)', () => {
      jest.useFakeTimers();
      expect(() => { UtilsService.reloadAfterUpdate(10); jest.advanceTimersByTime(20); }).not.toThrow();
      jest.useRealTimers();
    });
    ...
    it('fetchABIFromVerifiedContract calls open/send for the constructed URL', () => {
      ...
      class FakeXhr2 {
        readyState = 0; status = 0; responseText = ''; onreadystatechange: any = null;
        open(method: string, url: string, asyncFlag?: boolean) { opens.push({ method, url, asyncFlag }); }
        send() { sends++; }
      }
